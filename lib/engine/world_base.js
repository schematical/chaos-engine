	...
	app.WorldBase.prototype.load = function(cb){
		...
		return app.schema.WorldObject.find({
			...
			return async.eachSeries(
				objects,
			    function(object, cb){
					return app.Serializer.deserialize_object(object.serialized, function(err, object){
						if(err) throw err;
		...
		return async.series([
		    function(cb){
				...
				return app.Serializer.serialize_world(_this, function(err, world_data){
					if(err) throw err;
					_this._mongo_object.serialized = world_data;
				...
				return async.eachSeries(
					...
					function(npc_id, cb){
						...
						return app.Serializer.serialize_npc(npc, function(err, npc_data){
							if(err) throw err;
							npc._mongo_object.serialized = npc_data;
