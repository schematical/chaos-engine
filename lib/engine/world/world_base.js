...
module.exports = function(app){
	class WorldBase {
		...
		//This will get removed and we will just let our socket connection wait for NPCs
		cycle_npcs(cb) {
		...
		spawnRandom(spawn_func, count, tile_validatior_func) {
			var objects = []
				...
				return async.eachSeries(
					...
					function (object, cb) {
						return process.nextTick(function () {
							...
							//Remember only not stupid objects like NPC's are stored in WordObjects
							//But the dumb objects are stored in a giant JSON with the world
							return cb();
						});
					},
					function (errs) {
						...
						return async.eachSeries(
							...
							function (npc_id, cb) {
								...
								if (!npc) {
									...
									//throw new Error("Could not find NPC:" + npc_id);
								}
