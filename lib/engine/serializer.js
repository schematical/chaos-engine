...
module.exports = function(app){
	app.Serializer = class Serializer{
		static serialize_world(world, cb){
			var world_data = {
				tiles:{},
				objects:{}
			};
						...
						function(object_id, cb){
							...
							return process.nextTick(function(){
								...
								return object.trigger('serialize', object_data, function(){
									if(object_data.container){
										console.error(object_data);
										throw new Error("InventoryBaseObject(" + object_data.id + ") should not have a container");
									}
		...
		static serialize_object(object, cb){
			var object_data = {};
		...
		static getObjectClass(object_class){
		...
		static deserialize_object(world, object_data, cb){

			//Alter id
			console.log("Deserializing Object:" + object_data.id, object_data.inventory && Object.keys(object_data.inventory));
			try {
				var Class = app.Serializer.getObjectClass(object_data.object_class);
				if(!Class || !_.isFunction(Class)){
					throw new Error("Could not find valid class for `" + object_data.object_class + "`'");
				}
			}catch(e){
				console.error(object_data);
				throw e;
			...
			if(!_.isFunction(cloned_object.is_detached)){
		...
		static deserialize_world(data, cb){
		...
		static deserialize_npc(world, npc_data, cb){
			console.log("Deserializing Object:" + npc_data.id, npc_data.inventory && Object.keys(npc_data.inventory));
		...
		static deserialize_logicNodeChain(npc, logicNodeChain, cb){
