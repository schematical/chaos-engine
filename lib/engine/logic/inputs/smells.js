...
module.exports = function(app){
	app.logic.inputs.Smell = class Smell extends app.InputNodeBase{
		constructor(options){
			options.type = 'Smell';
			super(options);
			//Get close tiles
		...
		condition(data, cb){
			...
			for(var i in this.npc.world.objects){
				...
				if(!target_canidate.is_detached){
					console.error(target_canidate, this.npc.id);
					throw new Error("target_canidate Object has no method 'is_detached'");
				}
				if(
