			...
			if(data.interaction_type == 'mate'){
				...
				if(data.npc.gender == 'f'){
					...
					if(perk_base.npc.is_pregant){
						 console.log(data.npc.id + ' already pregnant. Gestation Ct:' + perk_base.npc.gestaton_ct);
						return cb();
					}
					...
					if(perk_base.npc.gestaton_ct == 0){
						...
						function(cb){
							return perk_base.npc.clone(function(err, cloned_npc){
								...
								cloned_npc.nourishment = cloned_npc.biology.nourishment_capacity  *.5;
								cloned_npc.age = 0;
								cloned_npc.markUpdated();
								console.log("Spawned:" , cloned_npc.id);
