...
module.exports = function(app){
	...
	app.NPCBase = function(options){
		...
		npc_base.interact = function(data){
			...
			console.log("User Interacting:", target_x, target_y);
		...
		npc_base.on('serialize',function(event, data, cb){
			...
			data.brain = {}
			data.brain.pleasure = this.brain.pleasure;
			data.brain.memory = this.brain.memory;//Got to work on this
			data.facing = this.facing;

			//TODO: Get this figured out
			data.brain.decision_matrix = {}
			for(var i in this.brain.decision_matrix){
				data.brain.decision_matrix[i] = this.brain.decision_matrix[i].serialize();
			}
			data.brain.pleasure_matrix = this.brain.pleasure_matrix;

			data.brain.biology = this.brain.biology;
			data.brain.equipped_slots = this.brain.equipped_slots;
			if(!data.inventory){
				return cb();
			}
			return async.eachSeries(
		...
		npc_base.on('unserialize',function(event, data, cb){
			...
			//TODO: Get this figured out
			this.brain.decision_matrix = {}
			for(var i in this.brain.decision_matrix){
				data.brain.decision_matrix[i] = data.brain.decision_matrix[i].serialize();
			}
			data.brain.pleasure_matrix = data.brain.pleasure_matrix;

			data.brain.biology = data.brain.biology;
			data.brain.equipped_slots = data.brain.equipped_slots;
			if(!data.inventory){
				return cb();
			}
			return async.eachSeries(
		...
		npc_base.clone = function(cb){
			...
			npc_base.trigger('serialize', npc_data, function(){
				...
				npc_data.id = npc_base.type +'-clone-' + npc_data.generation + '-' + Object.keys(npc_base.world.objects).length,
